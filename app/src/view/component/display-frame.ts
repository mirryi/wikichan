import { TemplateHandler } from "../template/template-handler";

import { Set, SortedSet } from "../../model/set";
import { WikiPage } from "../../model/page";
import { WikiLang } from "../../model/lang";

import * as callbacks from "./frame";

export class WikichanFrame {
    ...
    constructor() {
        ...
        this._width = WikichanFrame.DEFAULT_WIDTH;
        this._height = WikichanFrame.DEFAULT_HEIGHT;
    ...
    update(): void {
        ...
        this.articles.elements.forEach((a: WikiPage) => {
            ...
            div.getElementsByClassName("hide-button")[0].addEventListener("click", event =>
    ...
    filter(la: WikiLang): void {
        Array.from(this.responseContainer.getElementsByClassName(la.id)).forEach(
            (elem: HTMLElement) => {
